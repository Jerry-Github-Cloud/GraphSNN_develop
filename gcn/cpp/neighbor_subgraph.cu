...
void one_hop_subgraph_kernel(
    int num_hops, int* d_edge_index, int num_nodes, int num_edges, int* d_subgraph_nodes) {
    
    int node_idx = blockDim.x * blockIdx.x + threadIdx.x;
    if (node_idx < num_nodes) {
        // d_subgraph_nodes[0] = node_idx;
        d_subgraph_nodes[node_idx] ++;
        int idx = 0;

        for (int i = 0; i < num_edges; i ++) {
            if (node_idx == d_edge_index[2 * i]) {
                // d_subgraph_nodes[++ idx] = d_edge_index[2 * i + 1];
                d_subgraph_nodes[node_idx] ++;
            } else if (node_idx == d_edge_index[2 * i]) {
                // d_subgraph_nodes[++ idx] = d_edge_index[2 * i + 1];
                d_subgraph_nodes[node_idx] ++;
            }
        }
...
vector<int> get_all_neighbor_subgraph(const int num_nodes, int num_edges, vector<int>& edge_index, int num_hops) {
    cout << "num_nodes = " << num_nodes << endl;
    cout << "num_edges = " << num_edges << endl;

    int* d_edge_index;
    int* d_subgraph_nodes;

    cudaMalloc((void**)&d_edge_index, sizeof(int) * num_edges * 2);
    cudaMalloc((void**)&d_subgraph_nodes, sizeof(int) * num_nodes * num_hops);
    ...
    one_hop_subgraph_kernel<<<gridSize, blockSize>>>(num_hops, d_edge_index, num_nodes, num_edges, d_subgraph_nodes);

    vector<vector<int>> all_neighbor_subgraph;
    vector<int> subgraph_list(num_nodes, 0);
    cudaMemcpy(subgraph_list.data(), d_subgraph_nodes, sizeof(int) * num_nodes * num_hops, cudaMemcpyDeviceToHost);
    cudaFree(d_edge_index);
    cudaFree(d_subgraph_nodes);
...
int main() {
    ...
    clock_t start = clock();
    vector<int> subgraph_list = get_all_neighbor_subgraph(num_nodes, num_edges, edge_index, 1);
    clock_t end = clock();
    print_iterable(subgraph_list);
